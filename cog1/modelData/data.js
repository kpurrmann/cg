...
define(["exports", "data", "glMatrix"], function(data, exports) {
    ...
    function calculateNormalForPolygon(vertices, polygon, n){
        ...
        if (polygon.length < 3) {
            console.error("polygon does not have at least 3 vertices. Cannot compute normal");
            return [0, 0, 0];
        }

        var e1 = vec3.subtract(vertices[polygon[1]], vertices[polygon[0]], []);
        var e2 = vec3.subtract(vertices[polygon[2]], vertices[polygon[1]], []);
        ...
        if (d1 == d2 && d1 == d3) {
            console.error("segments are parallel; cannot calculate normal");
            return [0, 0, 0];
        }
